...
class ServerAPI:
    ...
    def delete_slices(self, name: str) -> bool:
        ...
        Returns:
            bool: True if successfull, else False.
        """
        payload = {"type": 9, "name": "delete_slices", "board": name}
        try:
            res = requests.put(self.adress, data=json.dumps(payload))
            return res.status_code == 204
        except ConnectionError as error:
    ...
    def get_active_model(self):
        ...
        payload = {"type": 5, "name": "active", "session": self.session}
        try:
            res = requests.get(self.adress, data=json.dumps(payload))
            if res.status_code == 200:
                return res.json()
    ...
    def train(self, slices: List[str], shape: Tuple[int, int], fit: dict):
        ...
        if not self.check_session_online():
            print(
                "Error: Old model couldn't be served again, please manually serve a model again"
            )
            return False
        payload = {
            "type": 2,
    ...
    def new_train(
        self,
        datasets: Tuple[List[str], List[str]],
        batch_size: Tuple[int, int],
        train_time: Tuple[int, int],
    ):
    ...
    def get_session(self):
        payload = {"type": 12, "name": "get_session"}
        try:
            res = requests.post(self.adress, data=json.dumps(payload))
            if res.status_code == 200:
                return res.json()
    ...
    def remove_session(self):
        if not self.session:
            return True
        payload = {"type": 13, "name": "remove_session", "session": self.session}
        try:
            res = requests.put(self.adress, data=json.dumps(payload))
            return res.status_code == 204
        except ConnectionError as error:
