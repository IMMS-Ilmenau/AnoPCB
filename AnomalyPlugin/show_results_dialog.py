...
class ShowResultsDialog(wx.Frame):
    ...
    def __init__(self, parent, plugin, layers, slice_positions, results):
        ...
        self.colour_pallete_8 = [wx.Brush(colour) for colour in [
        ...
        self.colour_pallete_20 = [wx.Brush(colour) for colour in [
        ...
        self.colour_pallete_40 = [wx.Brush(colour) for colour in [
        ...
        for layer in self.layers:
            image = wx.Image(
                self.im_size[0],
                self.im_size[1],
                Image.fromarray(layer, "L").resize(self.im_size).convert("RGB").tobytes())
            image.InitAlpha()
        ...
        for layer in self.layers:
            image = wx.Image(
                self.og_x_size,
                self.og_y_size,
                Image.fromarray(layer, "L").resize((
                    self.og_x_size,
                    self.og_y_size)).convert("RGB").tobytes())
            image.InitAlpha()
    ...
    def control_elements(self):
        ...
        self.kmeans_item = self.cluster_menu.AppendRadioItem(
            wx.ID_ANY,
            item="KMeans",
            help="choose KMeans as the clustering algorithm"
        )
        self.dbscan_item = self.cluster_menu.AppendRadioItem(
            wx.ID_ANY,
            item="DBscan",
            help="choose DBscan as the clustering algorithm"
        )
        self.optics_item = self.cluster_menu.AppendRadioItem(
            wx.ID_ANY,
            item="Optics",
            help="choose Optics as the clustering algorithm"
        )
        ...
        self.configuration_box = wx.Choice(self.grid_panel, choices=self.configurations[0])
        ...
        self.threshold_spin_text = wx.StaticText(self.threshold_panel, label="threshold: ")
        self.threshold_spin = wx.SpinCtrlDouble(self.threshold_panel, initial=0.02, inc=0.0001, max=10)
        self.cluster_spin_text = wx.StaticText(self.cluster_panel, label="cluster size: ")
    ...
    def control_logic(self):
        ...
        """
        self.Bind(wx.EVT_MENU, self.create_high_resolution_screenshot, self.save_screenshot_item)
        self.Bind(wx.EVT_MENU, self.create_high_resolution_screenshot_only_mask, self.save_screenshot_item_only_mask)
    ...
    def layouting(self):
        ...
        threshold_horizontal_sizer.Add(self.threshold_spin_text, 2, wx.ALIGN_CENTER_VERTICAL)
        threshold_horizontal_sizer.Add(self.threshold_spin, 5, wx.ALL | wx.EXPAND)
        cluster_horizontal_sizer.Add(self.cluster_spin_text, 2, wx.ALIGN_CENTER_VERTICAL)
    ...
    def change_layer(self, i):
        ...
        """
        try:
            bitmap = self.cached_result_bitmaps[(self.cluster_size, self.threshold, self.cluster_alg)][i]
        except:
    ...
    def view_results(self, event):
        ...
        try:
            self.cached_result_bitmaps[(self.cluster_size, self.threshold, self.cluster_alg)]
        except:
            ...
            new_layers_bitmaps = [
                wx.Bitmap(layer_base_bitmap) for layer_base_bitmap in self.layer_base_bitmaps]
            for new_layer_bitmap in new_layers_bitmaps:
                ...
                grc = wx.GraphicsContext.Create(memory_dc) # <- able to draw transparently :D
            ...
            if self.cluster_alg == ShowResultsDialog.Cluster_Alg.KMEANS:
                self.configurations[0].append(f"KMeans: threshold={key[1]} & cluster size={cluster_size}")
            elif self.cluster_alg == ShowResultsDialog.Cluster_Alg.DBSCAN:
                self.configurations[0].append(f"DBscan: threshold={key[1]} & cluster size={cluster_size}")
            else: # OPTICS
                self.configurations[0].append(f"Optics: threshold={key[1]} & cluster size={cluster_size}")
        ...
        self.configuration_box.SetSelection(
            self.configurations[1].index((self.cluster_size, self.threshold, self.cluster_alg)))
    ...
    def create_high_resolution_screenshot(self, event):
        ...
        with wx.FileDialog(
            ...
            new_layers_bitmaps = [
                wx.Bitmap(layer_base_bitmap_orig) for
                layer_base_bitmap_orig in self.layer_base_bitmaps_orig]
            bitmap_sum = wx.Bitmap(self.og_x_size, self.og_y_size*len(new_layers_bitmaps))
            dc_sum = wx.MemoryDC()
            dc_sum.SelectObject(bitmap_sum)

            for new_layer_bitmap, bitmap_sum_counter in zip(new_layers_bitmaps, range(len(new_layers_bitmaps))):
                print("start drawing a layer")
                memory_dc = wx.MemoryDC() # <- not able to draw transparently
                memory_dc.SelectObject(new_layer_bitmap)
                grc = wx.GraphicsContext.Create(memory_dc) # <- able to draw transparently :D
                ...
                for (x_pos, y_pos), mse in zip(self.slice_positions, self.results[1]):
                    ...
                    if y_pos > self.square_radius:
                        ...
                        y_pos = 0
                    # draw on graphics
                    grc.DrawRectangle(x_pos, y_pos, 1+2*self.square_radius-dx_pos, 1+2*self.square_radius-dy_pos)
                    # our clustering only considers values passing the threshold
                    cluster_type_counter += 1
                grc.Flush()
                dc_sum.Blit(
                    ...
                    self.og_x_size, self.og_y_size, memory_dc, 0, 0)
                memory_dc.SelectObject(wx.NullBitmap) # properly close DeviceController
    ...
    def update_configuration_box(self, key):
        ...
        if len(self.configurations[0]) > 1:
            self.configurations = list(map(list, zip(*list(sorted(
                list(zip(*self.configurations)), key=lambda x: (x[1][2], x[1][1], x[1][0]))))))
    ...
    def update_cluster_alg(self):
        self.cluster_alg = ShowResultsDialog.Cluster_Alg.KMEANS if self.kmeans_item.IsChecked() else (ShowResultsDialog.Cluster_Alg.DBSCAN if self.dbscan_item.IsChecked() else ShowResultsDialog.Cluster_Alg.OPTICS)
