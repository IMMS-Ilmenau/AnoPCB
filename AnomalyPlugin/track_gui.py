...
class TrackGUI(wx.Panel):
    ...
    def get_bitmap_icon(self, path):
        ...
        image = image.Scale(self.icon_width_height, self.icon_width_height, wx.IMAGE_QUALITY_HIGH)
    ...
    def filter_model(self, evt):
        ...
        Args:
            evt (wx.EVENT): Unused.
        """
        if not (os.path.isfile(self.plugin.anopcb_filter_slices_path)
            ...
            dia = wx.MessageDialog(
                ...
                caption="cannot show the filter results", style=wx.OK)
            dia.ShowModal()
            return
    ...
    def annotate_selected(self, evt):
        ...
        """
        try:
            comps = list(filter(lambda x: x.IsSelected(), list(self.get_tracks()) + list(self.get_pads())))
            # check if there's atleast one element
            next(iter(comps))
        except StopIteration:
            wx.MessageBox("Must select a component first!", 'Error', wx.OK | wx.ICON_ERROR)
            return
        ...
        signals = set()
        for signal in list(map(lambda x: self.plugin.get_annotated_net(x.GetNetCode()), comps)):
            signal = possible_signals[7] if signal is None else possible_signals[int(signal)-1] # we don't expect written signals with value 0
            signals.update({signal})
        
        # Erzeugung des Eingabefensters:
        signal = 8 if len(signals) != 1 else (possible_signals.index(next(iter(signals)))+1)
