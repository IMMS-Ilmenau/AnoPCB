...
class PreferencesWindow(wx.Frame):
    ...
    def control_elements(self):
        ...
        self.server_type_label = wx.StaticText(self.server_type_panel, label=" Server Type")
        self.server_type_choice = wx.Choice(self.server_type_panel, choices=self.server_type_choice_texts)
        self.save_filter_data_label = wx.StaticText(self.filter_panel, label="Save filter data locally?    ")
        self.save_filter_data_checkbox = wx.CheckBox(self.filter_panel)
        self.save_filter_data_checkbox.SetValue(prefs[5])
        self.empty_label = wx.StaticText(self, label="")#dont ask why its here, just accept that it is
        ...
        try:
            self.server_type_choice.SetSelection(self.server_type_choice_texts.index(prefs[4]))
        except:
            self.server_type_choice.SetSelection(self.server_type_choice_texts.index("remote"))
    ...
    def layouting(self):
        ...
        filter_horizontal_sizer.Add(self.save_filter_data_label, 2, wx.ALIGN_CENTER_VERTICAL)
        filter_horizontal_sizer.Add(self.save_filter_data_checkbox, 5, wx.ALL | wx.EXPAND)
        ...
        server_type_horizontal_sizer.Add(self.server_type_label, 2, wx.ALIGN_CENTER_VERTICAL)
    ...
    def load_prefs(self):
        ...
        # tuple #not anymore
        return [serveradd, serverport, slicex, slicey, server_type, save_filter_data, signal1, signal2, signal3, signal4, signal5, signal6, signal7, signal8]
    ...
    def save_prefs(self, event):
        ...
        server_type = self.server_type_choice_texts[self.server_type_choice.GetSelection()]
        ...
        new_signals = [signal1, signal2, signal3, signal4, signal5, signal6, signal7, signal8]
        while "None" in new_signals:
            new_signals.remove("None")

        if len(new_signals) > len(list(dict.fromkeys(new_signals))):
            wx.MessageBox(
                "No double names allowed (except None)",
                'Error',
                wx.OK | wx.ICON_ERROR)
            return
