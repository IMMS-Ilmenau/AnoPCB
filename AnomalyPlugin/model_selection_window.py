...
class ModelSelectionWindow(wx.Frame):
    ...
    def layouting(self):
        ...
        grid.Add(self.optimizer_label, pos=(0, 0), flag=wx.TOP | wx.LEFT | wx.BOTTOM, border=10)
        grid.Add(self.optimizer_text, pos=(0, 1), flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)
        grid.Add(self.loss_label, pos=(1, 0), flag=wx.TOP | wx.LEFT | wx.BOTTOM, border=10)
        grid.Add(self.loss_text, pos=(1, 1), flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)
        grid.Add(self.metrics_label, pos=(2, 0), flag=wx.TOP | wx.LEFT | wx.BOTTOM, border=10)
        grid.Add(self.metrics_text, pos=(2, 1), flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)
        grid.Add(self.send_button, pos=(3, 0), span=(1, 2), flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=5)
            ...
            if model[1] != "":
                ...
                if model[2] == "h5":
                    dlg = wx.MessageDialog(
                        None,
                        "Selected H5 file, compile parameters will be ignored.\nSend model \"" + model[1] + "\" to server?",
                        'send_event',
                        wx.YES_NO | wx.ICON_QUESTION)
                else:
                    dlg = wx.MessageDialog(
                        None,
                        "Send model \"" + model[1] + "\" with compile parameters to server?",
                        'send_event',
                        wx.YES_NO | wx.ICON_QUESTION)
                ...
                print(model[2])
                if result == wx.ID_YES:
                    self.plugin.server_api.send_model(model[0], model[1], model[2], {
